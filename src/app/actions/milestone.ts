...
export async function createmilestoneTask(formData: FormData) {
  try {
    ...
    let todoBoard = await prisma.milestoneBoard.findFirst({
      where: {
        projectId,
        title: "TODO",
      },
    ...
    todoBoard ??= await prisma.milestoneBoard.create({
      data: {
        title: "TODO",
        projectId,
        position: 0,
    ...
    const lastTask = await prisma.milestoneTask.findFirst({
    ...
    revalidatePath(`/milestone/${projectId}`);
...
export async function updatemilestoneTaskStatus(
  taskId: string,
  ...
  try {
    ...
    const updatedTask = await prisma.milestoneTask.update({
      where: { id: taskId },
      data: updateData,
      include: {
        assignee: { select: { fullname: true } },
    ...
    revalidatePath(`/milestone/${task.projectId}`);
    revalidatePath("/projects");

    return { success: true, task: updatedTask };
  } catch (error) {
    console.error("Error updating milestone task status:", error);
    return {
      success: false,
