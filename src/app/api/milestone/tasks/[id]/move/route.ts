/**
 * PATCH /api/milestone/tasks/[id]/move - Move task to different board or reorder
...
async function normalizeBoardPositions(tx: any, boardId: string) {
  const tasks = await tx.milestoneTask.findMany({
    where: { boardId },
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const newBoard = await prisma.milestoneBoard.findUnique({
      where: { id: newBoardId },
    });
