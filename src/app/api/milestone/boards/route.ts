/**
 * GET /api/milestone/boards - Get all boards for a project
 * POST /api/milestone/boards - Create a new board
 */
  ...
  try {
    ...
    if (project.status !== "APPROVED" && project.status !== "ACTIVE") {
      return NextResponse.json(
        { error: "milestone board only available for approved or active projects" },
        { status: 403 }
      );
    }

    // Fetch boards with tasks
    const boards = await prisma.milestoneBoard.findMany({
      where: { projectId },
      include: {
        tasks: {
          include: {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get max position
    const maxBoard = await prisma.milestoneBoard.findFirst({
      where: { projectId },
    ...
    // Create board
    const board = await prisma.milestoneBoard.create({
      data: {
        title,
        color: color ?? "#3b82f6",
