/**
 * PUT /api/milestone/boards/[id] - Update board (title, color)
 * DELETE /api/milestone/boards/[id] - Delete board
 * PATCH /api/milestone/boards/[id] - Reorder boards
 */
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const board = await prisma.milestoneBoard.findUnique({
      where: { id },
    ...
    // Update board
    const updated = await prisma.milestoneBoard.update({
      where: { id },
      data: {
        ...(title && { title }),
        ...(color && { color }),
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Delete board
    await prisma.milestoneBoard.delete({ where: { id } });

    // Reorder remaining boards
    const remainingBoards = await prisma.milestoneBoard.findMany({
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.milestoneBoard.findMany({
      where: { projectId: board.projectId },
      orderBy: { position: "asc" },
    });
    ...
    await prisma.$transaction(async tx => {
      if (newPosition > oldPosition) {
        // Moving right
        await tx.milestoneBoard.updateMany({
          where: {
      ...
      // Update moved board
      await tx.milestoneBoard.update({
        where: { id },
    ...
    const updatedBoards = await prisma.milestoneBoard.findMany({
      where: { projectId: board.projectId },
      include: {
        tasks: {
          include: {
